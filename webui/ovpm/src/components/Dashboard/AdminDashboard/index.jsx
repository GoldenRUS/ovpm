...
export default class AdminDashboard extends React.Component {
    ...
    handleGetNetworksFailure(error) {
        console.log(error);
        this.setState({ networks: [], isRefreshing: false });
    ...
    handleGetVPNStatusFailure(error) {
        console.log(error);
        this.setState({ vpn: {}, isRefreshing: false });
        if (error.response.status === 401) {
            this.handleAuthFailure(error);
        }
    }
    ...
    handleGetAuthStatusSuccess(res) {
        this.setState({ self: res.data.user, isRefreshing: false });
    }

    handleGetAuthStatusFailure(error) {
        console.log(error);
        this.setState({ self: {}, isRefreshing: false });
        if (error.response.status === 401) {
            this.handleAuthFailure(error);
        }
    }
    ...
    render() {
        ...
        const lastUpdateText = this.state.lastUpdate
                            ...
                            >
                                <Tab value="users" label="Users">
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            ...
                                            <small style={{ color: '#666' }}>{lastUpdateText}</small>
                                            <Button
                                                size="small"
                                ...
                                <Tab value="vpn" label="VPN">
                                    ...
                                    <table className="mui-table mui-table--bordered mui--text-justify">
                                        ...
                                        <tbody>
                                        <tr>
                                            <td>Hostname</td> <td>{this.state.vpn.hostname}</td>
                                        </tr>
                                        <tr>
                                            <td>Proto</td> <td>{this.state.vpn.proto}</td>
                                        </tr>
                                        <tr>
                                            <td>Port</td> <td>{this.state.vpn.port}</td>
                                        </tr>
                                        <tr>
                                            <td>Network</td>
                                            <td>
                                        ...
                                        <tr>
                                            <td>DNS</td> <td>{this.state.vpn.dns}</td>
                                        </tr>
                                        <tr>
                                            <td>Created At</td>
